...
vec1 <- c(3, 6, 3.4)   
```
Versuchen Sie aus verschiedenen Typen einen Vektor zu bilden, werden beide Werte zu dem Typen umgewandelt. Sie werden zu dem Typen umgewandelt, der in der Datentypen-Übersichtstabelle weiter oben steht bzw. die kleinere Nummer hat. Das nennt sich `coercion`:
```{r, echo=TRUE}
c("kreativ",3.5)  # Versucht `chr` und `num` zu einem Vektor zu kombinieren
```
...
```{r, error=TRUE}
as.character(c(1, TRUE, "abc", 4.1627))  # Verändert die Werte unterschiedlicher Typen zum Typ chr
```
```{r, error=TRUE}
as.double(c(2, TRUE, "abc", 4.1627))   # Coerced zum Typ num. Ist dies nicht möglich, erscheint NA   
```
...
```{r, echo=TRUE}
geschwister <- c(8,4,12)              # Def. der Variable geschwister
...
```{r}
# Erstelle die Matrix bsp_mat mit 4 Zeilen, 4 Spalten und leeren Einträgen. NaN (Not a Number) ist zwar vom Typ num, aber Berechnungen mit NaNs sind nicht so sinnvoll.

bsp_mat <- matrix(NaN, nrow=4, ncol=4) 
bsp_mat
```
Ich habe eine 4x4 Matrix erstellt, die mit NaNs gefüllt ist. Hätte ich diverse Datentypen zugeordnet, wären diese zum niedrigeren `coerced` worden. Matrizen können mit `matrixname[Zeilennummer,Spaltenummer]` manipuliert werden. Praktischerweise stehen die entspechenden Indizes neben der oben angezeigten Matrix. Beispiele zum Auswählen und Zuweisen neuer Werte:
```{r}
...
```
```{r}
bsp_mat[,3] <- c(FALSE, TRUE, FALSE, TRUE)  # Logi Werte werden zu num coerced
bsp_mat                                
```
...
```{r}
sav_mat <- bsp_mat    # Speichert bsp_mat unter sav_mat zur späteren Verwendung

bsp_mat[,4] <- c("coercion","kann","nervig","sein")  # Weist Spalte 4 einen chr-Vektor zu
bsp_mat

mode(bsp_mat)  # Bestimmt den Datentypen der neuen Matrix
```
...
```{r}
test_tib <- as_tibble(test_df)   # Konvertiert data.frame zu tibble
test_tib
```
...
tickets_2022$Prioritaet <- tickets_2022$Priorität  # Besser ae statt ä im Variablennamen 

tickets_2022                                       # Nun gibt es eine Spalte zu viel                          
```
```{r}
tickets_2022$Priorität <- NULL              # Entfernt eine Spalte, vorsichtig hiermit(!)
```
