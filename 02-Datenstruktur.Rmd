...
```{r, echo=TRUE}
# Ich versuche ein `chr` und eine `num` zu einem Vektor zu kombinieren
...
```{r, error=TRUE}
# Verändere eine Reihe von Werten zum Typ chr:

as.character(c(1, TRUE, "abc", 4.1627))  
```
```{r, error=TRUE}
# Verändere die Werte zum Typ num. Wo dies nicht möglich ist erscheinen NAs:
...
# Abruf des zweiten Elements des Vektors:
...
```
```{r}
# Logische Werte werden zu numerischen Werten coerced:
...
```{r}
# Wähle alle Zeilen von Spalte 1:

bsp_mat[,1]

# Wähle Zeile 4 von Spalte 1:

bsp_mat[4,1]      
```
Hier wird es turbulent:
```{r}
# Wähle Zeilen 1 & 3 von allen Spalten:

bsp_mat[c(1,3),] 

# Wähle alle Zeilen außer 1, und Spalten 2-4:

bsp_mat[-1,2:4]  
```
Da ich jetzt Bereiche der Matrix auswählen kann, kann ich vielleicht Berechnungen vornehmen:
```{r} 
# Ist Matrix numerisch, kann ich damit rechnen:
...
```{r}
# Summe über Zeile 1 ohne NaN:

sum(bsp_mat[1,], na.rm=TRUE)  

# MW der Matrix ohne NaN:
...
```{r}
# Speichere die Matrix unter anderem Namen zur späteren Verwendung:
...
# Bestimme den Datentypen der neuen Matrix:
...
```{r}
# Konvertiere den data.frame zu einem tibble test_tib:
...
# Berechne die Summe einer dieser Variablen:
...
```{r}
# Definiere neue Spalte im Datensatz:

tickets_2022$Priorität <- c(2, 4, 3, 5, 1)   

# besser ae statt ä in Variablennamen: 

tickets_2022$Prioritaet <- tickets_2022$Priorität

# nun gibt es eine Spalte zu viel:

tickets_2022                                      
```
```{r}
# entfernt eine Spalte, vorsichtig hiermit(!):

tickets_2022$Priorität <- NULL   
```
