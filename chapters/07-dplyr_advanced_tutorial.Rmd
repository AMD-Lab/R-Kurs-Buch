...
In R können wir das die Funktion `pivot_wider` machen lassen. Diese Funktion braucht zwei essentielle Information: Aus welcher Variable sollen die Variablennamen genommen werden? und: aus welche Variable sollen die Werte für die neuen Variablen genommen werden? Wir können es uns in unserem Beispiel oben so vorstellen: Wir nehmen die Variable `timepoint`, ziehen daraus alle möglichen Werte (`stress_t1`, `stress_t2` und `stress_t3`) und machen daraus wieder einzelne Variablen. Es ist aber noch unklar, woher die Werte für diese Variablen kommen sollen. Deshalb müssen wir noch angeben, dass sie aus der Variable `stress` gezogen werden sollen. In Code sieht das dann so aus:
