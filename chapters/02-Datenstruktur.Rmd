...
vec1 <- c(3, 6, 3.4)   
```
Versuchen Sie aus verschiedenen Typen einen Vektor zu bilden, werden sie zu dem komplexeren Typen von beiden umgewandelt. Die Datentypen-Übersichtsliste ist von einfach zu komplex (1-3) geordnet. Die Umwandlung des Datentypen nennt sich `coercion`:
```{r, echo=TRUE}
...
```{r}
# Bilde die Summe über Zeile 1 ohne NAs:
sum(bsp_mat[1,], na.rm=TRUE) 
# Bilde den MW der Matrix ohne NAs:
...
bsp_mat[,4] <- c("coercion","kann","tricky","sein")  
bsp_mat
...
>>>>>>> a7ff0a48d57a8359fafa84162254f2da21ab62f0
```
