...
Variablennamen können auch Umlaute und Unterstriche ("_") enthalten, sie dürfen aber nicht mit einer Zahl oder einem Unterstrich beginnen.
...
Wenn Sie in einer Variable Text abspeichern wollen, definieren Sie eine Variable vom Datentyp `character`. (Variablen vom Datentyp `character` werden auch oft *Strings* genannt.) Text wird mit doppelten oder einfachen Anführungszeichen angegeben:
...
Der Datentyp `logical` kodiert binäre Informationen -- diese sind entweder `TRUE` oder `FALSE`.
...
Wie mathematischen Ausdrücke auch, haben logische Operationen ebenfalls eine Rangfolge, nach denen sie ausgewertet werden. Daher kann es sinnvoll sein, Klammern `()` zu verwenden, um logische Operationen zu gruppieren.
